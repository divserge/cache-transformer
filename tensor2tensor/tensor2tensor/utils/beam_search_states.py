...
def beam_search(symbols_to_logits_fn,
  ...
  full_states = tf.expand_dims(states['state'], axis=1) if states else None
  full_states = tf.expand_dims(full_states, axis = 2) if states else None
  full_states = tf.tile(full_states, [1, beam_size, 1, 1])
  ...
  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, states, full_states):
    ...
    full_states = tf.concat([full_states, tf.expand_dims(states['state'], axis=2)], axis=2)
  ...
  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   finished_scores, finished_in_finished, unused_states, full):
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, states, full_states) = tf.while_loop(
       _is_finished,
       inner_loop, [
           tf.constant(0), alive_seq, alive_log_probs, finished_seq,
           finished_scores, finished_flags, states, full_states
       ],
       shape_invariants=[
           tf.TensorShape([]),
  ...
  return finished_seq, finished_scores, full_states
